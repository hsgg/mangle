...
c
      do i=1,np
        if (cm(i).eq.0._dp) goto 200
        if (cm(i).le.-2._dp) goto 200
      enddo
      gzeroar=1
      return
c
...
c-----------------------------------------------------------------------
      ...
      real(dp) rp(3,np),cm(np),rpi(3),cmi,xi(3),yi(3),big,tol,phi(2,np)
c
c        intrinsics
      intrinsic abs
c        local (automatic) variables
      integer j
      real(dp) bi,bj,cmij,cmj,d,dc,xj,yj
c *
...
c. . . . i circle is outside j circle
            if ((cm(j).ge.0._dp.and.bj.gt.0._dp)
     *        .or.(cm(j).lt.0._dp.and.bj.lt.0._dp)) then
c        j circle also outside i circle means null intersection area
              if ((scmi.ge.0.and.bi.gt.0._dp)
     *          .or.(scmi.lt.0.and.bi.lt.0._dp)) goto 220
...
c. . . . j circle is also inside i circle, and just touches it
              if ((scmi.ge.0.and.bi.lt.0._dp)
     *          .or.(scmi.lt.0.and.bi.gt.0._dp)) then
c        ph = atan(yj/xj) is angle from xi to rp(j)
                xj=xi(1)*rp(1,j)+xi(2)*rp(2,j)+xi(3)*rp(3,j)
                yj=yi(1)*rp(1,j)+yi(2)*rp(2,j)+yi(3)*rp(3,j)
...
c-----------------------------------------------------------------------
     ...
     *  jml,jmu,jpl,jpu,nphbv,jm(nphbv),jp(nphbv),km(nphbv),kp(nphbv)
      integer dp
      parameter (dp = 16)
      real(dp) rp(3,np),cm(np),rpi(3),cmi,tol,phi(2,np),phm,php,ph,dph
c
c        parameters
      include 'pi.par'
      real(dp) TWOPI
...
c        local (automatic) variables
      ...
      logical ismax
      real(dp) bik,cmik,cmk,d,psi,psim(2),dphc,si
...
c        dphc = azimuthal angle corresponding to great circle angle tol
      ...
      dphc=sin(tol/2._dp)/si
c        abort if tol/2 exceeds th(i)
      if (dphc.gt.1._dp) goto 300
      dphc=2._dp*asin(dphc)
...
c        psi = atan(sqrt(d)/bik) is exterior angle at intersection
            ...
            if ((scmi.ge.0.and.cm(kk).lt.0._dp)
     *        .or.(scmi.lt.0.and.cm(kk).ge.0._dp)) bik=-bik
            do iphbv=1,nphbv
...
c        choose largest exterior angle psi
                ...
                elseif (psi.ge.psim(iphbv)-tol) then
                  if (cm(kk).ge.0._dp) then
                    if (cm(km(iphbv)).ge.0._dp) then
...
c        or later of two degenerate circles
     ...
     *                  .and.kk.gt.km(iphbv)) then
                      ...
                      if (cm(kk)-1._dp.lt.cm(km(iphbv))+1._dp) then
                        ismax=.true.
                      elseif (cm(kk)-1._dp.eq.cm(km(iphbv))+1._dp
     *                  .and.kk.gt.km(iphbv)) then
                        ismax=.true.
                      endif
                    endif
                  else
                    if (cm(km(iphbv)).lt.0._dp) then
                      if (cm(kk).lt.cm(km(iphbv))) then
                        ismax=.true.
                      elseif (cm(kk).eq.cm(km(iphbv))
     *                  .and.kk.gt.km(iphbv)) then
...
c        or later of two degenerate circles
     ...
     *                  .and.kk.gt.kp(iphbv)) then
                      ...
                      if (cm(kk)-1._dp.lt.cm(kp(iphbv))+1._dp) then
                        ismax=.true.
                      elseif (cm(kk)-1._dp.eq.cm(kp(iphbv))+1._dp
     *                  .and.kk.gt.kp(iphbv)) then
                        ismax=.true.
                      endif
                    endif
                  else
                    if (cm(kp(iphbv)).lt.0._dp) then
                      if (cm(kk).lt.cm(kp(iphbv))) then
                        ismax=.true.
                      elseif (cm(kk).eq.cm(kp(iphbv))
     *                  .and.kk.gt.kp(iphbv)) then
...
c
c-----------------------------------------------------------------------
      real(16) function cmijf(rpi,rpj)
      integer dp
      parameter (dp = 16)
      real(dp) rpi(3),rpj(3)
c *
...
c        local (automatic) variables
      integer i,j
c *
c * Permutes elements of real(dp) array x(n) by permutation iperm(n).
c *
c  Input: n = dimension of array x
c         iperm = array of dimension n specifying permutation of x
c Input/Output: x = real(dp) array of dimension n
c Work array: wk of dimension n
c
      do i=1,n
        wk(i)=x(i)
...
c        local (automatic) variables
      integer i,j,k
c *
c * Permutes each column of real(dp) array x(m,n) by permutation iperm(n).
c *
c  Input: m,n = dimensions of array x
c         iperm = array of dimension n specifying permutation
c                 of each column x
c Input/Output: x = real(dp) array of dimension m,n
c Work array: wk of dimension n
c
      do k=1,m
        do i=1,n
...
c-----------------------------------------------------------------------
     ...
     *  whole,nbd0m,nbd0p,nbd,nmult,tol)
      ...
      integer dp
      parameter (dp = 16)
      real(dp) area,rp(3,np),cm(np),tol
c
c        parameters
      include 'pi.par'
      real(dp) TWOPI
...
c        check area does not exceed area within any one circle
        ...
        do i=1,np
          ...
          elseif (cm(i).lt.0._dp) then
            if (2._dp+cm(i).lt.cmmin) then
              cmmin=2._dp+cm(i)
              icmmin=i
            endif
          endif
...
c *
      if (tolin.le.0._dp) then
c        write(*,*) 'tolin =', tolin
        if (tol.le.0._dp) then
          tol=1.e-15_dp
c          write(*,*) 'tol =', tol
        else
          tol=tol*4._dp

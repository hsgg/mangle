...
!     a f 'f88 pix2vecnest.f'
     ...
     & vertex_w_x, vertex_w_y, vertex_w_z)
      ...
      INTEGER dp
      PARAMETER (dp = 16)
      REAL(dp) vector_x, vector_y, vector_z, vertex_n_x, 
     ...
     & ip_hi, jrt, jr, nr, jpt, jp, kshift, nl4, ns_max
      ...
      REAL(dp) z, fn, fact1, fact2, sth, phi, PI
...
!               if (id_mk.ge.2 .or. id_mk.lt.0) goto 20  ! bit value (in kpix), goes in iy_mk
      ...
      phi = (jp - (kshift+1)*0.5_dp) * (PI/(2.0_dp*nr))
      ...
      if (nr.gt.1) phi_up=(PI/2.0_dp)*(iphi_rat+iphi_mod/(DBLE(nr-1)))
      phi_dn         =(PI/2.0_dp)*(iphi_rat+(iphi_mod+1)/(DBLE(nr+1)))
